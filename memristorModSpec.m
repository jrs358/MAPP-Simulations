...
MOD = add_to_ee_model(MOD, 'parms',...
    {'f1_switch', f1_switch, 'f2_switch', f2_switch});
switch f1_switch
    case 1
...
switch f2_switch
    ...
    case 5
        MOD = add_to_ee_model(MOD, 'parms',...
            {'Vp', 0.16, 'Vn', 0.15, 'Ap', 4e3, 'An', 4e3});
        MOD = add_to_ee_model(MOD, 'parms',...
            {'xp', 0.3, 'xn', 0.5, 'alphap', 1, 'alphan', 5});
    case 6
        if 5 ~= f1_switch
            MOD = add_to_ee_model(MOD, 'parms', {'maxGap', 1.7, 'minGap', 0});
        end
        MOD = add_to_ee_model(MOD, 'parms',...
            {'Vel0', 10, 'Beta0', 0.8, 'gamma0', 16});
        MOD = add_to_ee_model(MOD, 'parms',...
            {'Ea', 0.6, 'a0', 0.25, 'tox', 12});
    otherwise
end
...
switch f1_switch
    ...
    case 3
        f1 = s^n*Beta*safesinh(Alpha*vpn, maxslope) + chi...
            *(safeexp(Gamma*vpn, maxslope)-1);
    case 4
        f1p = A1 * s * safesinh(B * vpn, maxslope);
        f1n = A2 * s * safesinh(B * vpn, maxslope);
        f1 = smoothswitch(f1n, f1p, vpn, smoothing);
    case 5
        f1 = I0*safeexp(-(s*minGap+(1-s)*maxGap)/g0, maxslope)...
            *safesinh(vpn/V0, maxslope);
    otherwise
end
...
switch f2_switch
    ...
    case 3
        i = fe(S);
        f2p = c_off * safesinh(i/i_off, maxslope) * safeexp...
            (-safeexp((s-a_off)/wc - i/b, maxslope) - s/wc, maxslope);
        f2n = c_on * safesinh(i/i_on, maxslope) * safeexp...
            (-safeexp(-(s-a_on)/wc + i/b, maxslope) - s/wc, maxslope);
        f2 = smoothswitch(f2n, f2p, i, smoothing);
    case 4
        Vstar = vpn-v_off+(v_off-v_on)*s;
        f2p = k_off * (Vstar/v_off)^alpha_off;
    ...
    case 5
        ...
        g_of_vpnp = Ap * (safeexp(vpn, maxslope) ...
            - safeexp(-Vn*s + Vp*(1-s), maxslope));
        g_of_vpnn = -An * (safeexp(-vpn, maxslope) ...
            - safeexp(+Vn*s - Vp*(1-s), maxslope));
        g_of_vpn = smoothswitch(g_of_vpnn, g_of_vpnp, Vstar, smoothing);
        
        f_of_sp = smoothswitch(1, safeexp(-alphap*(s-xp), maxslope), ...
            s-xp, smoothing);
        f_of_s = smoothswitch(safeexp(alphan*(s-1+xn), maxslope), ...
            f_of_sp, s-1+xn, smoothing);
    ...
    case 6
        ...
        Gamma = gamma0 - Beta0 * Gap^3;
        f2 = 1e9*(maxGap-minGap) * Vel0*exp(- q*Ea/k/T)*...
            safesinh(vpn*Gamma*a0/tox*q/k/T, maxslope);
    otherwise
end
